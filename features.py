...
class FeaturePipeline:
    ...
    def _sanitize_feature_df(self, df: pd.DataFrame, inferred_symbol: Optional[str] = None) -> pd.DataFrame:
        ...
        # If columns look like many date columns (wide format), transpose
        try:
            if self._looks_like_date_like_columns(list(df.columns)):
                logger.debug("Transposing dataframe because columns look date-like (wide->tall). shape before=%s", df.shape)
                df = df.T
        except Exception as e:
            logger.debug("Date-like heuristic failed: %s", e)
        ...
        if save and results:
            ...
            for i, df in enumerate(cleaned):
                ...
                try:
                    ...
                    logger.info("Aligned DF[%d] symbol=%s shape=%s", i, df['symbol'].iloc[0] if 'symbol' in df.columns else 'UNK', aligned_df.shape)
                except Exception as e:
