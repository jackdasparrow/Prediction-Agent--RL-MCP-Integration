...
class FeaturePipeline:
    ...
    # -------------------------
    # Helper / sanitization utils
    # -------------------------
    def _stable_seed_from_symbol(self, symbol: str) -> int:
        h = hashlib.md5(symbol.encode('utf-8')).hexdigest()
        return int(h[:8], 16)

    @staticmethod
    def _looks_like_date_like_columns(cols, threshold=0.5):
        """Return True if a substantial fraction of column labels look like datetimes/ISO strings."""
        if len(cols) == 0:
            return False
        date_like = 0
        for c in cols:
            # Accept pd.Timestamp, np.datetime64, or strings that parse as date-ish
            if isinstance(c, (pd.Timestamp, np.datetime64)):
                date_like += 1
            elif isinstance(c, str):
                # quick heuristic: year-like or contains '-' or '/'
                if (len(c) >= 4 and c[:4].isdigit()) or '-' in c or '/' in c:
                    date_like += 1
        return (date_like / len(cols)) >= threshold

    def _sanitize_feature_df(self, df: pd.DataFrame, inferred_symbol: Optional[str] = None) -> pd.DataFrame:
        """
        Ensure df is in tall format (rows = dates), has a Date column and symbol column.
        Return sanitized DataFrame (or raise on unrecoverable problems).
        """
        # Defensive copy
        df = df.copy()

        # If Series -> convert to DataFrame row
        if isinstance(df, pd.Series):
            df = df.to_frame().T

        # Avoid auto-transposing based on column heuristics.
        # Some feature names may contain dashes or numbers and be misidentified as date-like,
        # which would incorrectly transpose an already tall dataframe and break concatenation.
        # We therefore skip heuristic transposition here and assume inputs are tall (rows = dates).

        # If index is a DatetimeIndex (or named Date), turn index into Date column
        if isinstance(df.index, pd.DatetimeIndex) or (df.index.name and str(df.index.name).lower() in ("date", "index")):
            # Reset index; name it Date
            df = df.reset_index().rename(columns={df.index.name or "index": "Date"})
        else:
            # If there is a column that looks date-like already, ensure it's named Date
            date_col_candidates = [c for c in df.columns if isinstance(c, (pd.Timestamp, np.datetime64)) or (isinstance(c, str) and (len(c) >= 4 and c[:4].isdigit() or '-' in c or '/' in c))]
            if date_col_candidates:
                # pick the first candidate as Date
                candidate = date_col_candidates[0]
                if candidate != "Date":
                    df = df.rename(columns={candidate: "Date"})

        # If still no Date column but index is numeric and there are datetime-like values inside, try to coerce
        if "Date" not in df.columns:
            # Try to infer date from a column called 'date' lowercased
            for c in df.columns:
                if str(c).lower() == "date":
                    df = df.rename(columns={c: "Date"})
                    break

        # Ensure Date is of datetime dtype if present
        ...
        if "symbol" not in df.columns:
            if inferred_symbol is not None:
                ...
                # try to extract from attributes / name if available
                sym = getattr(df, "symbol", None) or getattr(df, "name", None)
                df["symbol"] = sym or "UNKNOWN"

        # Guarantee 2D DataFrame
        if df.ndim != 2:
            df = pd.DataFrame(df)
    ...
    # -------------------------
    # Column standardization & feature groups
    # -------------------------
    ...
    def compute_all_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Get feature columns (exclude OHLCV, metadata, targets, and timestamp columns)
        exclude_cols = [
    ...
    def process_universe(
        ...
        # Process each symbol and compute features
        ...
        # If saving, sanitize + align then save combined feature store
        if save and results:
            # Align per-symbol frames on common date index to avoid length mismatches
            cleaned = []
            for sym, df in results.items():
                try:
                    sanitized = self._sanitize_feature_df(df, inferred_symbol=sym)
                    cleaned.append(sanitized)
                except Exception as e:
                    logger.exception("Failed to sanitize feature df for %s: %s", sym, e)
                    # skip problematic df rather than failing everything
                    continue

            if len(cleaned) == 0:
                raise ValueError("No valid feature DataFrames after sanitization!")

            # Ensure a common Date column exists and is datetime
            for i in range(len(cleaned)):
                if 'Date' not in cleaned[i].columns:
                    # If index is datetime-like, promote it
                    if isinstance(cleaned[i].index, pd.DatetimeIndex):
                        cleaned[i] = cleaned[i].reset_index().rename(columns={cleaned[i].index.name or 'index': 'Date'})
                if 'Date' in cleaned[i].columns:
                    cleaned[i]['Date'] = pd.to_datetime(cleaned[i]['Date'], errors='coerce')

            # Compute common date intersection (drop NaT)
            date_sets = []
            for df in cleaned:
                if 'Date' in df.columns:
                    valid = df['Date'].dropna().unique()
                    date_sets.append(set(valid))
            if date_sets:
                common_dates = sorted(set.intersection(*date_sets)) if len(date_sets) > 1 else sorted(date_sets[0])
            else:
                common_dates = []

            # Trim each df to common date range if available, set Date as index to avoid
            # datetime extension array concat issues, then proceed
            trimmed = []
            for df in cleaned:
                dfx = df.copy()
                if 'Date' in dfx.columns:
                    if common_dates:
                        dfx = dfx[dfx['Date'].isin(common_dates)].sort_values('Date')
                    # Set Date as index for safe row-wise concatenation
                    try:
            ...
            # Align columns by union to ensure safe concatenation
            all_cols = []
            for df in trimmed:
                cols = list(df.columns)
                # Ensure Date is not included (now in index)
            ...
            # Reindex each sanitized df to have same columns (fill missing with NaN)
            aligned = []
            for i, df in enumerate(trimmed):
                # ensure columns order and include any missing columns
                try:
                    aligned_df = df.reindex(columns=all_cols_unique)
                    ...
                    if 'symbol' in df.columns:
                        try:
                            ...
                            sym_val = 'UNK'
                    logger.info("Aligned DF[%d] symbol=%s shape=%s", i, sym_val or 'UNK', aligned_df.shape)
                except Exception as e:
                    logger.exception("Failed to align df index %d: %s", i, e)

            # Now safe to concat row-wise, then restore Date as a column
            combined_df = pd.concat(aligned, axis=0, ignore_index=False)
            # Reset index if it's datetime so Date becomes a column
            if isinstance(combined_df.index, pd.DatetimeIndex):
                ...
                if 'index' in combined_df.columns and 'Date' not in combined_df.columns:
...
def main():
    ...
    for cache_file in cache_files:
        try:
            ...
            # If duplicate symbol present, keep the larger (more rows) dataset
            if symbol in raw_data:
                existing_rows = len(raw_data[symbol]) if getattr(raw_data[symbol], "__len__", None) else 0
                if len(df) > existing_rows:
                    logger.info("Replacing existing cached data for %s with larger file %s (%d rows -> %d rows)", symbol, cache_file.name, existing_rows, len(df))
                    raw_data[symbol] = df
                else:
                    logger.info("Keeping existing cached data for %s (existing %d rows >= new %d rows) - skipping %s", symbol, existing_rows, len(df), cache_file.name)
            else:
                raw_data[symbol] = df
